...
import {StratFeeManagerInitializable, IFeeConfig} from "../strategies/Common/StratFeeManagerInitializable.sol";
...
contract StrategyPassiveManagerUniswap is StratFeeManagerInitializable, IStrategyConcLiq, IStrategyUniswapV3 {
     ...
     */
    function lockedProfit() public view returns (uint256 locked0, uint256 locked1) {
    ...
    function range() external view returns (uint256 lowerPrice, uint256 upperPrice) {
        ...
        lowerPrice =
            FullMath.mulDiv(uint256(TickMath.getSqrtRatioAtTick(positionMain.tickLower)), SQRT_PRECISION, (2 ** 96)) **
                2;
        upperPrice =
            FullMath.mulDiv(uint256(TickMath.getSqrtRatioAtTick(positionMain.tickUpper)), SQRT_PRECISION, (2 ** 96)) **
                2;
    }
